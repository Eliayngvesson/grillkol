...
function normalizeOrder(
  ...
  return {
    ...
    customer_name: String(
      rawOrder.customer_name ?? "Okänd kund",
    ),
    phone: String(rawOrder.phone ?? ""),
    email:
      typeof rawOrder.email === "string"
        ? rawOrder.email
        : null,
    street_address: String(
      rawOrder.street_address ?? "",
    ),
    postal_code: String(
    ...
    latitude:
      typeof rawOrder.latitude === "number"
        ? rawOrder.latitude
        : rawOrder.latitude !== null &&
            rawOrder.latitude !== undefined
          ? Number(rawOrder.latitude)
          : null,
    longitude:
      typeof rawOrder.longitude === "number"
        ? rawOrder.longitude
        : rawOrder.longitude !== null &&
            rawOrder.longitude !== undefined
          ? Number(rawOrder.longitude)
          : null,
    route_position:
      rawOrder.route_position !== null &&
      rawOrder.route_position !== undefined
        ? Number(rawOrder.route_position)
        : null,
    delivery_date:
      typeof rawOrder.delivery_date ===
      "string"
        ? rawOrder.delivery_date
        : null,
    delivery_message:
      typeof rawOrder.delivery_message ===
      "string"
        ? rawOrder.delivery_message
        : null,
    route_name:
      typeof rawOrder.route_name === "string"
        ? rawOrder.route_name
        : null,
    planned_at:
      typeof rawOrder.planned_at === "string"
        ? rawOrder.planned_at
        : null,
    total_items: Number(
      rawOrder.total_items ?? 0,
    ),
    total_price: Number(
      rawOrder.total_price ?? 0,
    ),
    created_at: String(
      rawOrder.created_at ?? "",
    ),
...
export default function DeliveryPlanner() {
  ...
  const startPosition =
    useMemo<DeliveryMapPosition | null>(
      () => {
  ...
  const positionedSelectedOrders =
    useMemo(() => {
  ...
  const mapOrders = useMemo(() => {
    ...
    optimizedOrders.forEach(
      (order, index) => {
        optimizedPositions.set(
          order.id,
          index + 1,
        );
      },
    ...
    return orders.map((order) => ({
      ...order,
      route_position:
        optimizedPositions.get(order.id) ??
        null,
    }));
  ...
  const unpositionedSelectedCount =
  ...
  function toggleSelectedOrder(
    orderId: number,
  ) {
    setSelectedOrderIds(
      (currentSelectedIds) => {
        if (
          currentSelectedIds.includes(orderId)
        ) {
  ...
  function useCurrentPosition() {
    ...
    if (!navigator.geolocation) {
      setErrorMessage(
        "Din webbläsare stöder inte GPS-position.",
      );
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        setMessage(
          "Din nuvarande position används som ruttens startpunkt.",
        );
      },
      (error) => {
        if (error.code === 1) {
  ...
  async function placeOrderOnMap(
    ...
    if (error) {
      setErrorMessage(
        `Kartpositionen kunde inte sparas: ${error.message}`,
      );
    ...
    setMessage(
      "Kundens leveransplats har sparats.",
    );

    setSaving(false);
  }

  async function removeOrderPosition(
    orderId: number,
  ) {
    ...
    if (error) {
      setErrorMessage(
        `Kartpositionen kunde inte tas bort: ${error.message}`,
      );
    ...
    setMessage(
      "Kartpositionen har tagits bort.",
    );
  ...
  async function markOrderAsDelivered(
    ...
    if (error) {
      setErrorMessage(
        `Beställningen kunde inte markeras som levererad: ${error.message}`,
      );
  ...
  function openGoogleMapsRoute() {
    ...
    if (optimizedOrders.length === 0) {
      setErrorMessage(
        "Välj minst en beställning med sparad kartposition.",
      );
      return;
    }
    ...
    if (!routeUrl) {
      setErrorMessage(
        "Google Maps-rutten kunde inte skapas.",
      );
      return;
    }
    ...
    <main className="delivery-planner-page">
      <header className="delivery-planner-header">
        <div>
          ...
          <p>
            Välj beställningar, placera
            leveransadresser på kartan och skapa
            en smart körordning.
          </p>
        </div>
        ...
        <section className="delivery-orders-card">
          ...
          {loading ? (
            ...
            <div className="delivery-empty">
              ...
              <strong>
                Inga beställningar att planera
              </strong>

              <p>
                Nya, bekräftade och planerade
                beställningar visas här.
              </p>
            </div>
          ) : (
        ...
        <section className="delivery-map-card">
          {mapPlacementOrderId !== null && (
            <div className="delivery-map-instruction">
              Klicka på kundens exakta
              leveransplats på kartan
            </div>
          )}
          ...
          <div className="delivery-route-list">
            ...
            {optimizedOrders.length === 0 ? (
              <p className="delivery-route-empty">
                Välj beställningar med
                kartposition för att skapa en
                körordning.
              </p>
            ) : (
              <ol>
